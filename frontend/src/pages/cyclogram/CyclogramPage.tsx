...
function SlotRow({ slot, timePlaceholder, activityPlaceholder, onChange, onRemove }: SlotFormProps) {
  return (
    <div className="flex items-center gap-2">
      <input type="text" placeholder={timePlaceholder} value={slot.time ?? ''}
        onChange={(e) => onChange({ ...slot, time: e.target.value })} className="input w-32 text-sm" />
      <input type="text" placeholder={activityPlaceholder} value={slot.activity ?? ''}
        onChange={(e) => onChange({ ...slot, activity: e.target.value })} className="input flex-1 text-sm" />
      <button type="button" onClick={onRemove} className="text-gray-300 hover:text-red-500 transition-colors shrink-0">
...
export default function CyclogramPage() {
  ...
  const MONTH_NAMES = Array.from({ length: 12 }, (_, i) => t(`cyclogram.month.${i}`))

  const DAYS: { key: DayOfWeek; label: string }[] = [
    { key: 'monday',    label: t('cyclogram.day.monday') },
    { key: 'tuesday',   label: t('cyclogram.day.tuesday') },
    { key: 'wednesday', label: t('cyclogram.day.wednesday') },
    { key: 'thursday',  label: t('cyclogram.day.thursday') },
    { key: 'friday',    label: t('cyclogram.day.friday') },
  ]
  ...
  const createMutation = useMutation({
    mutationFn: () => cyclogramApi.create({
      organizationId: organizationId!, branchId: activeBranch as any, groupId: activeGroup,
      academicYear, month, week, language: 'ru', content: stringifyContent(editContent),
    }),
  ...
  const updateMutation = useMutation({
    mutationFn: (content: CyclogramContent) =>
      cyclogramApi.update(editCyclogramId!, { organizationId: organizationId!, content: stringifyContent(content) }),
  ...
  const addSlot = useCallback((day: DayOfWeek) => {
    setEditContent((prev) => ({ ...prev, [day]: [...(prev[day] ?? []), { time: '', activity: '' }] }))
  }, [])
  ...
  const renderContent = (raw: string | null) => {
    ...
    if (!hasData) return <Empty message={t('cyclogram.empty')} />
    return (
          ...
          <div key={key} className="rounded-lg border border-gray-200 overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {(content[key] ?? []).length === 0 ? (
                <p className="px-3 py-2 text-xs text-gray-400 italic">{t('cyclogram.dayEmpty')}</p>
              ) : (
        ...
        <div key={key}>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={() => addSlot(key)}
              ...
              <Plus className="h-3 w-3" /> {t('cyclogram.addSlot')}
            </button>
          </div>
          <div className="space-y-2">
            {(editContent[key] ?? []).map((slot, idx) => (
              <SlotRow key={idx} slot={slot}
                timePlaceholder={t('cyclogram.slotTimePlaceholder')}
                activityPlaceholder={t('cyclogram.slotActivityPlaceholder')}
                onChange={(s) => updateSlot(key, idx, s)}
                onRemove={() => removeSlot(key, idx)} />
            ))}
            {(editContent[key] ?? []).length === 0 && (
              <p className="text-xs text-gray-400 italic">{t('cyclogram.noSlots')}</p>
            )}
          </div>
        </div>
      ))}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{t('cyclogram.title')}</h1>
          {activeGroupObj && (
            <p className="text-sm text-gray-500 mt-0.5">{t('cyclogram.group')}: {activeGroupObj.name}</p>
          )}
        </div>
        {canEdit && activeGroup && !current && (
          <Button onClick={() => { setEditContent({}); setCreateModalOpen(true) }}>
            <Plus className="h-4 w-4" /> {t('cyclogram.create')}
          </Button>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-end gap-3">
        {branches.length > 0 && (
          <Select label={t('cyclogram.branch')}
            options={branches.map((b) => ({ value: b.id, label: b.name }))}
            value={branchId} onChange={(e) => { setBranchId(e.target.value); setGroupId('') }}
            placeholder={t('cyclogram.selectBranch')} className="w-44" />
        )}
        <Select label={t('cyclogram.group')}
          options={groups.map((g) => ({ value: g.id, label: g.name }))}
          value={groupId} onChange={(e) => setGroupId(e.target.value)}
          placeholder={t('cyclogram.selectGroup')} className="w-44" />
        <Input label={t('cyclogram.academicYear')} value={academicYear}
          onChange={(e) => setAcademicYear(e.target.value)} placeholder="2025-2026" className="w-28" />
      ...
      {!activeGroup ? (
        <Empty message={t('cyclogram.selectGroupHint')} />
      ) : isLoading ? (
        <Spinner />
      ) : !current ? (
        <Empty message={`${MONTH_NAMES[month - 1]}, ${t('cyclogram.week')} ${week} — ${t('cyclogram.notCreated')}`} />
      ) : (
        <div className="space-y-3">
          <div className="flex items-center justify-between bg-white rounded-xl border border-gray-200 px-4 py-3">
            <div className="flex items-center gap-3">
              <span className={clsx('px-2.5 py-0.5 rounded-full text-xs font-semibold',
                current.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700')}>
                {current.status === 'approved' ? t('cyclogram.status.approved') : t('cyclogram.status.draft')}
              </span>
              <span className="text-sm text-gray-500">
                {MONTH_NAMES[current.month - 1]}, {current.week} {t('cyclogram.week')} • {current.academicYear}
              </span>
              {current.generatedByAi && (
                <span className="px-2 py-0.5 rounded-full text-xs bg-purple-100 text-purple-700 font-medium">AI</span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {canApprove && current.status === 'draft' && (
                <Button size="sm" variant="secondary" onClick={() => approveMutation.mutate(current.id)} loading={approveMutation.isPending}>
                  <CheckCircle className="h-4 w-4" /> {t('cyclogram.approve')}
                </Button>
              )}
              {canEdit && current.status !== 'approved' && (
                <Button size="sm" variant="secondary" onClick={() => openEdit(current)}>
                  <Pencil className="h-4 w-4" /> {t('cyclogram.edit')}
                </Button>
              )}
              {canApprove && (
                <Button size="sm" variant="danger"
                  onClick={() => { if (confirm(t('cyclogram.deleteConfirm'))) deleteMutation.mutate(current.id) }}
      ...
      <Modal open={editModalOpen} onClose={() => setEditModalOpen(false)}
        title={t('cyclogram.editTitle')} width="max-w-3xl">
        {renderEditForm()}
        <div className="flex gap-3 justify-end pt-4 border-t border-gray-100 mt-4">
          ...
          <Button onClick={() => updateMutation.mutate(editContent)} loading={updateMutation.isPending}>
            {t('common.save')}
          </Button>
        </div>
      </Modal>
      ...
      <Modal open={createModalOpen} onClose={() => setCreateModalOpen(false)}
        title={`${t('cyclogram.create')} — ${MONTH_NAMES[month - 1]}, ${t('cyclogram.week')} ${week}`}
        width="max-w-3xl">
        {renderEditForm()}
        <div className="flex gap-3 justify-end pt-4 border-t border-gray-100 mt-4">
